...
exports.book_delete_get = asyncHandler(async (req, res, next) => {
  // Get details of book and all their booksInstances (in parallel)
  const [book, bookInstances] = await Promise.all([
...
exports.book_delete_post = asyncHandler(async (req, res, next) => {
  ...
  const [book, allBooksByInstance] = await Promise.all([
    Book.findById(req.params.id).exec(),
    BookInstance.find({ book: req.params.id }, "title summary").exec(),
  ]);

  if (allBooksByInstance.length > 0) {
    // Author has books. Render in same way as for GET route.
    res.render("book_delete", {
      title: "Delete Book",
      book: book,
      instance_books: allBooksByInstance,
    });
    return;
  } else {
